        ...
        <article class=" p-2 shadow-lg">
            ...
            <div class="bg-white p-2 my-3">
              <div class="w-100 text-right">
                <a href="{{route('reports.details.create',$report->id)}}" class="border p-1 rounded " style="background: #51131c !important; color:white !important; margin-top:5px !important;"><i class="fa fa-plus"></i> ثبت عنوان</a>
              </div>
              @foreach ($report->details as $detail)
                ...
                <div class="text-center border border-top-1 rounded text-sm ">
                  ...
                  <a href="{{route('reports.details.edit',[$report,$detail->id])}}" class="mx-2"><i class="fa fa-edit text-gray"></i></a>
                </div>
              @endforeach
            </div>
  ...
  <div class="col-12">
    <div class="my-3 ">
      <a href="{{route('report.image.create',$report->id)}}" class="btn btn-success my-1">ثبت عکس</a>
        ...
        <a href="{{route('report.file.download',$report->id)}}" class="btn btn-info my-1">دانلودفایلها</a>
        @endif
        <a href="{{route('report.print',$report->id)}}" target="_blank" class="btn btn-warning text-white my-1">آماده چاپ</a>

        <a href="{{route('report.edit',$report->id)}}" class="btn btn-primary my-1">اصلاح</a>
...
<div class="modal fade" id="delete_report_modal">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
    <form action="{{route('report.destroy',0)}}" id="delete_report_form" method="post">
      @csrf
      @method('DELETE')
      <!-- Modal Header -->
      <div class="modal-header bg-danger">
...
<div class="modal fade" id="delete_title_modal">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
    <form action="{{route('reports.details.destroy',[$report->id,0])}}" id="delete_title_form" method="post">
      @csrf
      @method('DELETE')
      <!-- Modal Header -->
      <div class="modal-header bg-danger">
